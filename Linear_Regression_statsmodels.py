...
def LinRegAutoMPG():
    ...
    ssr_in_sample = np.sum((y - y_pred_in_sample)**2)
    ssr_split = np.sum((y - y_pred_split)**2)
    ...
    # Mean Absolute Error
    mae_in_sample = np.mean(np.abs(y - y_pred_in_sample))
    mae_split = np.mean(np.abs(y - y_pred_split))

    # Symmetric Mean Absolute Percentage Error (SMAPE)
    smape_in_sample = np.mean(2 * np.abs(y - y_pred_in_sample) / (np.abs(y) + np.abs(y_pred_in_sample))) * 100
    smape_split = np.mean(2 * np.abs(y - y_pred_split) / (np.abs(y) + np.abs(y_pred_split))) * 100  
    ...
    print("\\begin{table}[h]")
    print("\\centering")
    print("\\caption{Statsmodels - House Price Linear Regression}")
    print("\\label{tab:Statsmodels - House Price Linear Regression}")
...
def LinReginsurance():
    ...
    print("\\begin{table}[h]")
    print("\\centering")
    print("\\caption{Statsmodels - Insurance Charges Linear Regression}")
    print("\\label{tab:Statsmodels - Insurance Charges Linear Regression}")
    print("\\begin{tabular}{|c|c|c|}\\hline")
    print("Regression & In-Sample & 80-20 Split \\\\ \\hline \\hline")
    print(f"rSq & {r_squared_in_sample:.4f} & {r_squared_split:.4f} \\\\ \\hline")
    print(f"rSqBar & {r_squared_adj_in_sample:.4f} & {r_squared_adj_split:.4f} \\\\ \\hline")
