...
\title{Project 1}
\author{Manager: Brendan McDonnell \phantom{thisisatest}\\
Masum Billah\\
Madhu Chencharapu\\
Gabriel Loos\\
Roshan Ravichandran}
...
The report will be organized as follows,\Cref{EDA} goes over the EDA, \Cref{Linear Regression} will be focused on linear regression, \Cref{Regularized Regression} regularized regression, \Cref{Transformed Regression} transformed regression, and \Cref{Comp_mods} will be comparing all of the models.

In \Cref{Linear Regression}, in-sample, out-of-sample, 5-fold cross validation, and feature selection results are given for multiple linear regression on all three data sets. The results come from both scalation in scala and statsmodels in python. \Cref{Regularized Regression} goes over in-sample and out-of-sample quality of fit metrics and plots for Ridge Regression and Lasso. \Cref{Transformed Regression} gives in-sample, out-of-sample, and feature selection results for the Sqrt, Log1p, and Box-Cox transformations.

\input{Sections/EDA}

\input{Sections/Linear_Regression}

\input{Sections/Regularized_Regression}

\input{Sections/Transformed_Regression}

\input{Sections/Comp_mods}
